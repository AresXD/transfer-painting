...
const OriginalColor = {
  ...
  labBoundary (pin, pout) {
    ...
    if (OriginalPalette.distance2(pin, pout) < 0.001 || this.isEqual(RGBin, RGBout)) {
      return mid
    }
  ...
  labIntersect (p1, p2) {
    if (this.isOutLab(p2)) {
      ...
      return this.labIntersect(p2, OriginalPalette.add(p2, OriginalPalette.sub(p2, p1)))
    }
  }
}
...
const ReferralColor = {
  ...
  labBoundary (pin, pout) {
    ...
    const RGBout = this.lab2rgb(pout)
    const RGBmid = this.lab2rgb(mid)
    if (ReferralPalette.distance2(pin, pout) < 0.001 || this.isEqual(RGBin, RGBout)) {
      return mid
    }
    if (this.isOutRGB(RGBmid)) {
      return this.labBoundary(pin, mid)
  ...
  labIntersect (p1, p2) {
    if (this.isOutLab(p2)) {
      ...
      return this.labIntersect(p2, ReferralPalette.add(p2, ReferralPalette.sub(p2, p1)))
    }
  }
}
